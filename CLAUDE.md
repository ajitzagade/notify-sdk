...
Known gap, not yet fixed (found during a later review, out of scope of the feature work that found it): `NotifyClient.optIn()`/`optOut()` persist the consent change to storage first, then send a WhatsApp confirmation message **without** a try/catch. If that confirmation send throws (bad token, rate limit, transient outage), the exception propagates up through `handleInboundMessage`, so the webhook responds non-2xx to Meta — Meta then retries the same STOP/START webhook, redundantly (but harmlessly, since the DB write is idempotent) re-running opt-in/opt-out until the confirmation send succeeds or Meta's retry window lapses. If you're in this file anyway, wrapping those two `http.post` calls the same way the read-receipt one is wrapped would close it — just don't bundle it silently into an unrelated change.
